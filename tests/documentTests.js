...
window.testSuite.load(new TestingClass('Document', 'documentTests.js')
	...
	.test({
		description: 'isInGoogleDrive() - true',
		run: function () {},
		assert: function () {
			return this.doc.isInGoogleDrive === true;
		}
	})
	.test({
		description: 'isInGoogleDrive() - false',
		run: function () {
			var that = this;
			gapi.drive.realtime.newInMemoryDocument(function (doc) {
				that.newInMemoryDocument = doc;
			}, function () {});
		},
		assert: function () {
			return this.newInMemoryDocument.isInGoogleDrive === false;
		}
	})
	// .test({
